...
function SlotCard({ slot, order }: { slot: number; order?: Order }) {
  ...
  return (
    <div className={`rounded-2xl border-2 flex flex-col h-44 ${order ? 'bg-white border-amber-300 shadow' : 'bg-gray-100 border-gray-200'}`}>
...
export default function CafePage() {
  ...
  const slots = Array.from({ length: 10 }, (_, i) => ({ slot: i + 1, order: preparing.find(o => o.cafeSlot === i + 1) }));
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-amber-50">
      <header className="bg-amber-500 text-white px-6 py-3 flex items-center gap-4 shrink-0">
        ...
        <span className={`text-xl font-black px-4 py-1 rounded-full ${preparing.length >= 8 ? 'bg-red-500' : 'bg-amber-700'}`}>{preparing.length}건</span>
