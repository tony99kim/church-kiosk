...
function SlotCard({ slot, order }: { slot: number; order?: Order }) {
  ...
  return (
    <div className={`rounded-2xl border-2 flex flex-col min-h-48 ${
      ...
      <div className={`flex items-center justify-between px-3 pt-2.5 ${order ? 'text-green-600' : 'text-gray-300'}`}>
      ...
      {order ? (
        <>
          <div className="flex-1 flex flex-col items-center justify-center px-2">
            <span className={`text-6xl font-black leading-none ${isReady ? 'text-green-600' : 'text-green-300'}`}>{order.id}</span>
          ...
          {isReady ? (
            ...
            <div className="mx-2 mb-2.5 py-2.5 rounded-xl bg-green-100 text-green-400 text-base font-bold text-center">
              준비 중...
            </div>
          )}
        </>
...
function OverflowCard({ order }: { order: Order }) {
  ...
  return (
    <div className={`rounded-2xl border-2 p-4 flex items-center gap-4 ${
      isReady ? 'bg-white border-green-500' : 'bg-green-50 border-green-300'
    }`}>
      <span className={`text-4xl font-black shrink-0 ${isReady ? 'text-green-600' : 'text-green-300'}`}>{order.id}</span>
      ...
      {isReady ? (
        ...
        <span className="shrink-0 px-5 py-2.5 rounded-xl bg-green-100 text-green-400 text-base font-bold">
          준비 중...
        </span>
      )}
    </div>
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-green-50">
      <header className="bg-green-600 text-white px-6 py-4 flex items-center gap-4 shrink-0">
        <h1 className="text-3xl font-black">🍱 음식 수령대</h1>
        <span className={`text-2xl font-black px-4 py-1 rounded-full ${active.length >= 8 ? 'bg-red-500' : 'bg-green-800'}`}>{active.length}건</span>
