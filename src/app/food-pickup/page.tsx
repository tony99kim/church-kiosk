...
function SlotCard({ slot, order }: { slot: number; order?: Order }) {
  ...
  return (
    <div className={`rounded-2xl border-2 flex flex-col h-44 ${
      order ? 'bg-white border-green-400 shadow-md' : 'bg-gray-100 border-gray-200'
    }`}>
      ...
      {order ? (
        <>
          <div className="flex-1 flex items-center justify-center">
            <span className="text-6xl font-black text-green-600">{order.id}</span>
          </div>
...
export default function FoodPickupPage() {
  ...
  const overflow = ready.filter(o => !o.foodPickupSlot);

  const slots = Array.from({ length: 10 }, (_, i) => ({
    slot: i + 1,
    order: ready.find(o => o.foodPickupSlot === i + 1),
  }));
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-green-50">
      <header className="bg-green-600 text-white px-6 py-3 flex items-center gap-4 shrink-0">
        ...
        <span className={`text-xl font-black px-4 py-1 rounded-full ${ready.length >= 8 ? 'bg-red-500' : 'bg-green-800'}`}>
          {ready.length}건
        </span>
        {overflow.length > 0 && (
