...
function OverflowCard({ order }: { order: Order }) {
  ...
  return (
    <div className={`rounded-2xl border-2 p-3 flex items-center gap-3 ${
      isReady ? 'bg-white border-green-400' : 'bg-green-50 border-green-200'
    }`}>
      <span className={`text-3xl font-black shrink-0 ${isReady ? 'text-green-600' : 'text-green-300'}`}>{order.id}</span>
