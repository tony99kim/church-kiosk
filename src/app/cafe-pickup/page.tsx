...
export default function CafePickupPage() {
  ...
  const slots = Array.from({ length: 10 }, (_, i) => ({ slot: i + 1, order: ready.find(o => o.cafePickupSlot === i + 1) }));
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-amber-50">
      <header className="bg-amber-500 text-white px-6 py-3 flex items-center gap-4 shrink-0">
        ...
        <span className={`text-xl font-black px-4 py-1 rounded-full ${ready.length >= 8 ? 'bg-red-500' : 'bg-amber-700'}`}>{ready.length}건</span>
