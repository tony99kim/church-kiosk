...
function SlotCard({ slot, order }: { slot: number; order?: Order }) {
  const ready = () => order && fetch(`/api/orders/${order.id}/food-ready`, { method: 'POST' });
      ...
      {order ? (
        <>
          <div className="flex-1 flex flex-col items-center justify-center px-2">
            ...
            {optLines.map((s, i) => <span key={i} className="text-sm font-bold bg-sky-100 text-sky-700 px-2 py-0.5 rounded-full text-center mt-1 leading-snug">{s}</span>)}
          </div>
          {isReady ? (
...
export default function FoodPage() {
  ...
  const overflow = active.filter(o => !o.foodSlot);
  const slots = Array.from({ length: 10 }, (_, i) => ({ slot: i + 1, order: active.find(o => o.foodSlot === i + 1) }));
  const preparingCount = active.filter(o => o.foodStatus === 'preparing').length;
